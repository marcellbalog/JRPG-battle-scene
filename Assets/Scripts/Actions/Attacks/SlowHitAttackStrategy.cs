...
{
	// Variation of the punch attack with slower movement
	public class SlowHitAttackStrategy : MonoBehaviour, IAttackStrategy
	{
		public void ExecuteAttack(AttackContext context)
		{
			transform.DOMove(context.attackTarget.position, 1f / 30f * Vector3.Distance(transform.position, context.attackTarget.position))
						.SetEase(Ease.Linear)
						.OnComplete(() =>
						{
